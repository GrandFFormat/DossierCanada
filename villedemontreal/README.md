...
Le volet Québec vit dans le dépôt de DossierQuébec sous `villedequebec/` et est servi à
`dossierquebec.ca/villedequebec/`. Ce dossier a été construit dans le dépôt DossierCanada (c'est
là que la session avait le droit de pousser) et est **prêt à être déplacé tel quel** dans le
dépôt DossierQuébec sous `villedemontreal/` : rien dedans ne dépend de son dépôt hôte, sauf
trois choses à emporter avec lui — le workflow `.github/workflows/refresh-villedemontreal.yml`,
les entrées `headers`/`redirects` de `vercel.json`, et les lignes de `.vercelignore` et de
`.gitignore`. Le bouton « ← Retour à DossierQuébec » et l'adresse du User-Agent supposent déjà
`dossierquebec.ca/villedemontreal/`.

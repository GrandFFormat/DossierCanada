...
REM    2. trouve le depot DossierCanada sur ce PC (ou le telecharge dans
REM       Documents\DossierCanada s'il n'y est pas) ;
...
set "BRANCHE=claude/inspiring-ride-t99opb"
set "DEPOT_URL=https://github.com/GrandFFormat/DossierCanada"
...
if not defined VOLET if exist "%USERPROFILE%\Documents\DossierCanada\villedemontreal\scrapers\decisions.js" set "VOLET=%USERPROFILE%\Documents\DossierCanada\villedemontreal\"
if not defined VOLET if exist "%USERPROFILE%\DossierCanada\villedemontreal\scrapers\decisions.js" set "VOLET=%USERPROFILE%\DossierCanada\villedemontreal\"

if not defined VOLET (
  echo  Le depot DossierCanada n'est pas sur ce PC : telechargement dans
  echo  "%USERPROFILE%\Documents\DossierCanada" ...
  git clone --branch "%BRANCHE%" "%DEPOT_URL%" "%USERPROFILE%\Documents\DossierCanada"
  if errorlevel 1 (
    echo  [!] Le telechargement du depot a echoue. Verifiez la connexion Internet.
    goto :fin_erreur
  )
  set "VOLET=%USERPROFILE%\Documents\DossierCanada\villedemontreal\"
)
